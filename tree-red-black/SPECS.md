...
Purpose
- [ ] Create empty red-black tree with comparator-defined ordering.
...
- [ ] Red-black recolor and rotation rules restore invariants before operation completes.
...
- [ ] Delete of root and near-root nodes is supported.
- [ ] BST ordering and red-black invariants remain valid after deletion.
...
### `Clone() *Tree[T]`
Purpose
- [ ] Create independent red-black tree with same sorted contents and comparator.
...
Purpose
- [ ] Create independent red-black tree while optionally transforming each live value.
...
- [ ] `Color() Color` returns node color (`ColorBlack` or `ColorRed`).
...
- [ ] `Clone()` and `CloneWith(...)` preserve `Len()`, comparator, sorted order, and red-black invariants in clone.
...
- [ ] `RootNode/NodeAPI` tests for empty/non-empty root access, left-right child order, color exposure, child-count consistency, and early stop in `Children()`.
...
- [ ] Suggested safety factor for stable simple red-black ops is `1.75x`.
...
- Validate `RootNode/NodeAPI` child ordering, color values, child-count consistency, and full-node coverage.
